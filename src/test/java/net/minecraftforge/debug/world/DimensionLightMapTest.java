...
public class DimensionLightMapTest
{
    ...
    public void onDimensionLightMapModification(DimensionLightMapModificationEvent event)
    {
        ...
        {
            //Make the colors flash red
            float red = 1F;
            float green = event.getColors().getX() * 1 - ((float)(Math.sin(event.getWorld().getGameTime() * 0.1)) * 0.5F);
            float blue = event.getColors().getX() * 1 - ((float)(Math.sin(event.getWorld().getGameTime() * 0.1)) * 0.5F);
            event.getColors().set(red, green, blue);
        }
    }
